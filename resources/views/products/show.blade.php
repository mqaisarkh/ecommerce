      ...
      @foreach($product->images as $image)
        @if($loop->index === 0)
          <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
            <img src="{{ asset('storage/' . $image->path) }}" alt="First image" class="hidden size-full rounded-lg object-cover lg:block">
        @elseif($loop->index === 1)
            <div class="hidden lg:grid lg:grid-cols-1 lg:gap-y-8">
              <img src="{{ asset('storage/' . $image->path) }}" alt="Second image" class="aspect-3/2 w-full rounded-lg object-cover">
        @elseif($loop->index === 2)
              <img src="{{ asset('storage/' . $image->path) }}" alt="Third image" class="aspect-3/2 w-full rounded-lg object-cover">
            </div>
        @elseif($loop->index === 3)
            <img src="{{ asset('storage/' . $image->path) }}" alt="Fourth image" class="aspect-4/5 size-full object-cover sm:rounded-lg lg:aspect-auto">
          </div>
        @endif
      @endforeach
